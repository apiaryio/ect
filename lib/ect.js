	...
	var	ECT = function (options) {
		...
		TemplateContext.prototype.load = function (template, layout) {
			...
			if (typeof template === 'function') {
				// template = function(){} => direct render allowed
				container = { file : file, compiled : template };
				return container;
			}
			else if ((typeof template === 'string') && (typeof ect.options.root === 'object') && (typeof ect.options.root[template] === 'function')) {
				// root = {
				//  'template/path': function(){},
				//  'another/file': function(){}
				// }
				container = { file : file, compiled : ect.options.root[template] };
				return container;
			}
			else {
